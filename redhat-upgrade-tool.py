...
def check_release_version_file():
    ...
    try:
        ...
        raise SystemExit(1)
    if rel != args.network:
        print _("The upgrade to the system version %s is not allowed. The"
                " Preupgrade Assistant has assessed the system for upgrade to"
                " the version %s." % (rel, args.network))
        raise SystemExit(1)
